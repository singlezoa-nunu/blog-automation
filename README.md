...
이렇게 하면 끝이에요. 이후로는 **매일 한국시간 오전 8:30, 저녁 9시에 두 블로그 각각에 자동으로 글이 올라갑니다 (하루 2편씩).** 컴퓨터를 꺼두셔도 상관없어요 — GitHub의 서버에서 대신 돌아갑니다.
...
**Q. 예약 발행도 되나요?**
네! `config/blogs.js`에서 블로그마다 `publishSchedule` 값(예: `[{hour:8, minute:30}, {hour:20, minute:30}]`)을 설정해두면, 그 시각마다 각각 한 편씩 Blogger에 "예약됨" 상태로 등록됩니다. 실제 스크립트 실행은 그보다 미리(자정) 이루어지고, 정해진 시각에 자동으로 발행돼요. Blogger 관리 화면에 들어가 보시면 "예약됨"이라고 표시된 걸 확인하실 수 있어요. 시간을 늘리거나 줄이고 싶으면 배열에 항목을 추가/삭제하면 됩니다 (예: 하루 3편으로 늘리기).

**Q. 제휴 상품 링크는 어떻게 넣나요?**
글 끝에 "👉 오늘의 추천 제품 보러가기" 문구가 자동으로 붙어서 나갑니다 (`config/blogs.js`의 `ctaText`에서 수정 가능). 발행된 글마다 다루는 상품이 다르기 때문에 링크까지 자동으로 걸 수는 없어서, Blogger 에디터에서 그 문구를 마우스로 선택 → 링크 아이콘 클릭 → 상품 URL 붙여넣기만 해주시면 됩니다.

**Q. 예전에 수동으로 썼던 글이랑 주제가 겹치지 않나요?**
매 실행마다 그 블로그에 있는 기존 글 제목을 전부 조회해서, 자동화 이전에 수동으로 쓰신 글까지 포함해 겹치지 않는 주제를 고르도록 되어 있어요. 그래도 완벽하진 않을 수 있으니, 가끔 비슷한 주제가 보이면 알려주세요.
